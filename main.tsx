...
// --- FALLBACK / DEMO DATA ---
// Used if registry.json cannot be loaded (e.g., file not created yet)
const DEMO_REGISTRY = [
  { 
    id: 'demo-welcome', 
...
2. **Check Paths**: Ensure the filename in \`registry.json\` matches your actual file exactly.
  `
};
...
export default function PersonalWiki() {
  ...
  // 2. Load Article Content when activeArticle changes
  useEffect(() => {
    ...
    const loadContent = async () => {
      ...
      // If we are in registry fallback mode, don't try to fetch real files unless they exist in demo
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-6 custom-scrollbar">
          {registryError && (
             <div className="px-4 mb-4">
               <div className="bg-amber-50 dark:bg-amber-900/20 text-amber-800 dark:text-amber-200 p-3 rounded text-xs border border-amber-200 dark:border-amber-800">
                 <strong>Setup Mode</strong><br/>
                 Could not load <code>registry.json</code>. Showing demo content.
               </div>
             </div>
          )}
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800">
           <div className="bg-indigo-50 dark:bg-slate-800 rounded-lg p-3 text-xs text-slate-600 dark:text-slate-400 flex gap-2">
             ...
             <p>Edit <code>registry.json</code> to add new articles.</p>
           </div>
        </div>
      </aside>
